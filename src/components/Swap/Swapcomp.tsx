...
import { fundEscrow as fundingEscrow } from "./utils/fundEscrow";
import bs58 from "bs58"
// import fundEscrow from "./utils/fundEscrow";
  ...
  const fetchUserAssets = async function () {
    ...
    try {
      ...
      const [fungibleTokens, nonFungibleTokens] = await Promise.all([
        getFungibleTokensForWallet(publicKey.toString()),
        getNonFungibleTokensForWallet(publicKey.toString())
      ]);
  ...
  const handleSwap = async () => {
    ...
    try {
      if (isNftToToken) {
        ...
        // await fundEscrow({
        //   umi: swapUmi,
        //   tokenMintAddress: selectedToken.mintAddress.toString(),
        //   escrowPubKey: escrowPublickKey.toString()
        // })


        // await fundingEscrow({
        //   umi: swapUmi,
        //   escrowConfigurationAddress: convertToPublicKey(escrowPublickKey),
        //   tokenMint: convertToPublicKey(selectedToken.mintAddress),
        //   amount: 10_000
        // })
