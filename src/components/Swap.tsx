  ...
  const handleSwap = async () => {
    ...
    try {
      if (isNftToToken) {
        ...
        const selectedNft = availableNftMints.find(
          (nft) => nft.mint === selectedNftMint
        );
        if (selectedNft?.isLocked) {
          updateToastError(
            toastId,
            new Error("Selected NFT is currently locked")
          );
          return;
        }

        await handleNftToTokenSwap(selectedNftMint);
        ...
        await handleTokenToNftSwap(availableNft.mint);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        <div className="space-y-4">
              ...
              <div className="text-sm text-gray-400 flex items-center gap-1">
                ...
                {isMetadataLoading ? (
                  ...
                  <div className="flex items-center">
                    <span>
                      {isNftToToken
                        ? `${availableNftMints.filter((nft) => !nft.isLocked)
                          .length
                        } $ASTRO`
                        : `${(
                          tokenBalance / 1000000000
                        ).toLocaleString()} $MOONL`}
                    </span>
                    {((isNftToToken && availableNftMints.length === 0) ||
                      (!isNftToToken && tokenBalance === 0)) && (
          ...
          {((isNftToToken && Number(nftAmount) > 0) || !isNftToToken) &&
            connected && (
              <div className="bg-gray-700/50 rounded-lg p-4">
                <div className="flex justify-between mb-2">
                  <label className="text-sm text-gray-400">
          ...
          {connected ? (
            <motion.button
              ...
              className={`w-full py-3 bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-lg text-gray-900 font-bold flex items-center justify-center gap-2 ${isLoading || isBalanceLoading || isMetadataLoading
